...
function promedioResultadosTest(resultadosTest) {
	...
	if (resultadosTest.length === 0) {
		return 0; // Si el arreglo está vacío, retornamos 0 para evitar división por cero.
	}
...
function cuentoElementos(array) {
	...
	for (let i = 0; i < array.length; i++) {
		if (array[i] > 18) {
			contador++; // Si el elemento es mayor que 18, incrementamos el contador.
		}
	}
...
function todosIguales(array) {
	...
	// Tu código:for (let i = 1; i < array.length; i++) {
...
function continueStatement(num) {
	...
	for (let i = 1; i <= 10; i++) {
		if (i === 5) {
			continue; // Saltar la quinta iteración
		}
